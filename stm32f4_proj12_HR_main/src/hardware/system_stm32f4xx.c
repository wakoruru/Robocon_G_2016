...
#if !defined(HSE_VALUE) || (HSE_VALUE < 1000000UL)
 #error HSE Value is Not Defined!!
#else 
...
#if defined (STM32F40_41xxx)     
	/* STM32F4xxx Chip ID & Revision Detection Nemui added */
	uint16_t devid,revid;
	devid = DBGMCU_GetDEVID();
	revid = DBGMCU_GetREVID();

	/* STM32F40x RevA have Errata, DeviceID has 0x0411(Same as STM32F2)! */
    if((revid == 0x2000)&&(devid == 0x0411))
	{
	    /* STM32F40x RevA Chip S**K! */
